...
(ns advent-of-code-2023.day4.exercise2
  (:require [clojure.pprint :refer [pprint]]
            [clojure.string :as str]
            [clojure.math :as math]))

(defn parse-numbers [numbers-raw]
  (let [split-numbers (str/split numbers-raw #"\s+")]
    (mapv #(Integer/parseInt %) split-numbers)))

(defn parse-cards-data [cards-data]
  (mapv
    (fn [card-data]
      (let [[card-id-data card-numbers-data] (str/split card-data #":\s+")
            [winning-numbers-raw have-numbers-raw] (str/split card-numbers-data #"\s+\|\s+")
            card-id (Integer/parseInt (str/replace card-id-data #"Card\s+" ""))
            winning-numbers (parse-numbers winning-numbers-raw)
            have-numbers (parse-numbers have-numbers-raw)]
        {:id card-id
         :winning-numbers winning-numbers
         :have-numbers have-numbers}))
    cards-data))

(defn get-matching-numbers [winning-numbers have-numbers]
  (filterv #(.contains winning-numbers %) have-numbers))

(defn add-matching-numbers-to-card [card]
  (let [winning-numbers (:winning-numbers card)
        have-numbers (:have-numbers card)
        matching-numbers (get-matching-numbers winning-numbers have-numbers)]
    (assoc card :matching-numbers matching-numbers)))

(defn determine-next-cards [cards index cards-count]
  (let [card (get cards index)
        id (:id card)
        won-count (count (:matching-numbers card))
        start-index (min (dec cards-count) id)
        end-index (min (dec cards-count) (+ id won-count))]
    (println "adding " (- end-index start-index) " cards for card with id " id " and " index " index")
    (subvec cards start-index end-index)))

(defn compute-all-won-cards [cards-count initial-cards]
  (loop [cards initial-cards
         index 0]
    (if (= index (count cards))
      cards
      (let [next-cards (determine-next-cards cards index cards-count)]
        (recur
          (into [] (concat cards next-cards))
          (inc index))))))

(defn sum-card-ids [cards-count cards]
  (loop [index 0
         result []]
    (if (= index (count cards))
      result
      (recur
        (inc index)
        (let [{id :id} (get cards index)
              result-index (dec id)
              current-sum (if (> (count result) result-index) (get result result-index) 0)]
          (assoc result result-index (inc current-sum)))))
    ))

(defn exercise2 [cards-data]
  (let [cards (parse-cards-data cards-data)
        cards-count (count cards)]
    (->> cards
           (mapv add-matching-numbers-to-card)
           (compute-all-won-cards cards-count)
           (sum-card-ids cards-count)
           (reduce +))))

(defn -main []
  (pprint (exercise2 ["Card   1: 82 15 37 75 85 51 99 18 17  2 |  8 17 54 14 18 99  4 85 51 49 91 15 82 24 75 25 69 61 52 58 37 87  2 22 28"
