;; --- Day 2: Cube Conundrum --- You're launched high into the atmosphere! The
;; apex of your trajectory just barely reaches the surface of a large island
;; floating in the sky. You gently land in a fluffy pile of leaves. It's quite
...
(ns advent-of-code-2023.day2.exercise1
  (:require [clojure.pprint :refer [pprint]]
            [advent-of-code-2023.day2.common :refer [parse-game-data]]))

(defn possible-round? [{:keys [rounds]}]
  (every?
    (fn [{:keys [blue red green]}]
      (and (< blue 15)
           (< green 14)
           (< red 13)))
    rounds))

(defn exercise1 [games]
  (->> games
      (mapv parse-game-data)
      (filter possible-round?)
      (mapv (fn [game] (:id game)))
      (reduce +)))

(defn -main []
  (pprint (exercise1 ["Game 1: 4 red, 5 blue, 9 green; 7 green, 7 blue, 3 red; 16 red, 7 blue, 3 green; 11 green, 11 blue, 6 red; 12 red, 14 blue"
