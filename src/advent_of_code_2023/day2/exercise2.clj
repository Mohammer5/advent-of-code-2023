...
;; - Game 2 could have been played with a minimum of 1 red, 3
;;   green, and 4 blue cubes.
;; - Game 3 must have been played with at least 20
;;   red, 13 green, and 6 blue cubes.
;; - Game 4 required at least 14 red, 3 green,
;;   and 15 blue cubes.
...
(ns advent-of-code-2023.day2.exercise2
  (:require [clojure.pprint :refer [pprint]]
            [advent-of-code-2023.day2.common :refer [parse-game-data]]))

(defn find-highest-colors [rounds]
  (reduce (fn [acc {:keys [blue red green]}]
            {:blue (max (:blue acc) blue)
             :green (max (:green acc) green)
             :red (max (:red acc) red)})
          rounds))

(defn exercise2 [games]
  (->> games
      (mapv parse-game-data)
      (mapv #(:rounds %))
      (mapv find-highest-colors)
      (mapv (fn [{:keys [blue red green]}] (* blue red green)))
      (reduce +)))

(defn -main []
  (pprint (exercise2 ["Game 1: 4 red, 5 blue, 9 green; 7 green, 7 blue, 3 red; 16 red, 7 blue, 3 green; 11 green, 11 blue, 6 red; 12 red, 14 blue"
