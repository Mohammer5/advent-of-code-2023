...
(defn determine-next-char-digit [characters]
  (let [first-char (first characters) ;(subs character 0 1))
    ...
    (if (number? char-as-int)
      char-as-int
      (->> word-number-mapping
          (map (fn [[word number]] (when (string/starts-with? characters word) number)))
          (filter some?)
          (first)))))

(defn extract-digits [characters]
  (loop [digits []
         restCharacters characters]
    (if (= 0 (count restCharacters))
      digits
      (recur
        (let [next-char-digit (determine-next-char-digit restCharacters)]
          (if (number? next-char-digit)
            (conj digits next-char-digit)
            digits))
        (subs restCharacters 1)))))

(defn first-and-last-digit [digits]
  (let [f (first digits)
        l (last digits)]
    (vector f l)))

(defn as-two-digit-number [[first-number last-number]]
  (Integer. (str first-number last-number)))

(defn extract-digit-from-line [line]
  (->> line
         (extract-digits)
